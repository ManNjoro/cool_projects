...
export default function ReportsScreen({ navigation }) {
  ...
  const exportToExcel = async () => {
    try {
      ...
      dailyCowData.forEach(day => {
        day.cows.forEach(cow => {
          dailyCowWSData.push({
            ...
            'Cow Name': cow.name,
            'Total Litres': cow.litres,
            'Morning': cow.entries.find(e => e.time.toLowerCase() === 'morning')?.litres || 0,
            'Afternoon': cow.entries.find(e => e.time.toLowerCase() === 'afternoon')?.litres || 0,
            'Evening': cow.entries.find(e => e.time.toLowerCase() === 'evening')?.litres || 0
          });
        });
      });
      ...
      const filename = `milk_report_${new Date().toISOString().split('T')[0]}.xlsx`;
              ...
              try {
                if (Platform.OS === 'android') {
                  ...
                  if (permissions.granted) {
                    ...
                    Alert.alert("Success", "Report downloaded successfully to your Downloads folder");
                  }
                } else {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity 
      ...
      onPress={() => setSelectedDate(item.date === selectedDate ? null : item.date)}
    >
  ...
  const renderDailyCowItem = ({ item }) => (
    <View style={styles.cowProductionItem}>
      ...
      <View style={styles.cowProductionDetails}>
        ...
        <Text style={styles.cowProductionTotal}>
          Total: {item.litres}L
        </Text>
      </View>
    </View>
  ...
  const renderTopCowsSection = () => (
    <View style={styles.summarySection}>
      ...
      <FlatList
        ...
        ListFooterComponent={
          cowData.length > 3 ? (
            <Text style={styles.moreText}>+ {cowData.length - 3} more cows</Text>
          ) : null
        }
      />
    </View>
